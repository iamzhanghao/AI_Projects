...
def sgdLogReg(X, y, l=0, step_size=0.01, w0=None, max_iter=1000,
    ...
    def f(w, Xj, yj):
        ...
        score = - (np.sum(np.multiply(yj, np.log(z)) + \
                          np.multiply(1.0 - yj, np.log(1.0 - z)))) \
                + l * float(w.T * w)
        # score = yj*np.log(z) + (1.0-yj)*np.log(1.0-z)
        return score
