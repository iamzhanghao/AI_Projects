...
def gd(f, df, x0, step_size=.01, max_iter=1000, eps=.00001):
    ...
    for i in range(max_iter):
        xs.append(xs[-1] - step_size * df(xs[-1]))
        fs.append(f(xs[-1]))
        x = xs[-1]
        if abs(np.linalg.norm(xs[-1]) - np.linalg.norm(xs[-2])) < eps:
            break
...
def t6(learning_rates=(.0001, .001, .01, .05, .07, .075, .1)):
    ...
    for lr in learning_rates:
        ...
        plotData(ax, list(range(len(scores))), scores, style=None,
                 label=str(lr))
        print('lr', lr, 'w', w.T, 'err', scores[-1])
        dmin = min(-1.5, np.min(vals));
        dmax = max(1.5, np.max(vals))
...
def t7(learning_rates=(.0001, .001, .01, .1, .5, 1)):
    ...
    for lr in learning_rates:
        ...
        plotData(ax, list(range(len(smoothScores))), smoothScores,
